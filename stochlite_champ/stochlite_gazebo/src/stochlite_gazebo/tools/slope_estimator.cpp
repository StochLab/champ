...
#include "stochlite_gazebo/tools/rotation.h"
#include "stochlite_gazebo/tools/tof_coordinates.h"
...
namespace stochlite {
	...
	double giveMedian(double a[6], int n)
	{
	...
	Vector3f slope_estimator(std::vector<double> z, std::vector<double> imu_r_p) {
		...
		Vector3f diag12, diag23, diag34, diag45, diag56, diag61;
		...
		diag61 = tof1 - tof6;
		// Normal to the plane
		plane_normal1 = cross(diag61, diag12);
		plane_normal2 = cross(diag12, diag23);
		plane_normal3 = cross(diag23, diag34);
		plane_normal4 = cross(diag34, diag45);
		plane_normal5 = cross(diag45, diag56);
		plane_normal6 = cross(diag56, diag61);
	...
	Vector3f normalToEuler(Vector3f plane_normal, double n, std::vector<double> imu_r_p)
	{
		...
		Vector3f y_esti_world{0,0,0};
		Vector3f x_esti_world{0,0,0};

	#if 1	
		double imu_rot[3][3]= {
					{ cos(imu_r_p.at(1)),     sin(imu_r_p.at(1))*sin(imu_r_p.at(0)),  sin(imu_r_p.at(1))*cos(imu_r_p.at(0))},
		...
		// Vector3f y_est; y_est = cross(plane_normal,vx);
		// Vector3f x_est; x_est = cross(y_est,plane_normal);

		for(int j = 0; j < 3; j++)
		{
			for(int k = 0; k < 3; k++)
			{
				// y_esti_world[j] += imu_rot[j][k] * y_est[k];
				// x_esti_world[j] += imu_rot[j][k] * x_est[k];
		...
		// Matrix3f rot_mat_support_plane; rot_mat_support_plane = transpose(x_esti_world,y_esti_world,plane_normal_world);
